...
describe('findByIdAndUpdate', () => {
  ...
  it('will correctly update only the Blogpost with the Category the user has access to', async () => {
    ...
    // Execute
    const blogPost = await BlogPost.findByIdAndUpdate(originalQuery, originalPayload, { MongoBouncer, new: true });
  ...
  it('will not modify anything if the user does not have access', async () => {
    ...
    // Execute
    const blogPost = await BlogPost.findByIdAndUpdate(originalQuery,originalPayload, { MongoBouncer });
