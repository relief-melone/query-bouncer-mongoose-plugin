...
describe('updateMany', () => {
  ...
  it('will correctly update only the Blogposts with the Category the user has access to', async () => {
    // Prepare
    const originalQuery = { };
    const originalPayload = { Title: 'A new BlogPost' };
    ...
    // Execute
    const blogPost = await BlogPost.updateMany(originalQuery, originalPayload, { MongoBouncer });
  ...
  it('will not modify anything if the user does not have access', async () => {
    ...
    // Execute
    const blogPost = await BlogPost.updateOne(originalQuery,originalPayload, { MongoBouncer });
